// ADS Touch Sensor Test Example Program (IC P/N:ANMG04 )
// Code:
// Date: 2021.02.26  Ver.: 0.0.2
...
// ============= ANMG04 I2C Slave Address ============================//
#define ANMG04_ID 0x24 //0x48>>>1( 0b0100100 7bit + R/W 1 Bit , 7bit=0x24, 8bit=0x48)
// ============= ANMG04 I2C Slave Address ============================//

void  Init_ANMG04(void); //Initialize ANMG04
...
void Register_Dump()
{
   ...
   for (int i = 0; i < 256; i += 16)
   {
      for (int j = 0; j <= 15; j++)
      {
         Wire.beginTransmission(ANMG04_ID); // sned ic slave address
         Wire.write((i + j));                // sends register address
         Wire.endTransmission();             // stop transmitting
         Wire.requestFrom(ANMG04_ID, 1);    // data read (2 byte)
...
void loop() {
   ...
   Wire.endTransmission();             // stop transmitting
   Wire.requestFrom(ANMG04_ID, 1);    // key data read (1 byte)
...
void  Init_ANMG04(void)
{
   // Register setting values are subject to change without prior notice to improve touch operation.
   //------------------ Software Reset Enable (Set)----------------------
   Wire.beginTransmission(ANMG04_ID); // 
